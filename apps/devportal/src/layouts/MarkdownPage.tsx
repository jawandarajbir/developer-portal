...
// Components
import MarkdownContent, { MarkdownPageContent } from '@/src/components/common/MarkdownContent';
...
const MarkdownPage = ({ hasGrid, pageInfo, partials, promoAfter, promoBefore, customNav, customNavPager }: GenericContentPageProps) => {
  if (!partials) {
          ...
          <Container>
            <div className="mt-8 grid-cols-4 gap-6 md:grid">
              ...
              <div className={pageInfo.hasInPageNav ? 'col-span-3' : 'col-span-4'}>
                <MarkdownPageContent partials={partials} />
                {customNavPager}
              </div>
            </div>
          </Container>
